...
| Responsables    | Juan Polanco, Marlon Sanabria, Michael Martinez                                   |
...
| -------- | -------- | ------------------------ | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------ |
| CP-01 📷 | Positivo | Inventario con 3 equipos | Opción 1 → código `EQ-004`, tipo `Portátil`, marca `HP`, modelo `ProBook 440` | Mensaje `Equipo 'EQ-004' registrado correctamente.`, estado inicial *Disponible*, y el equipo aparece en `equipos.json`      | ![Captura CP-01](Imagenes/1.png)                                              | **C**  |
| CP-02    | Negativo | EQ-001 ya existe         | Opción 1 → código `EQ-001` + cualquier dato                                   | Mensaje `Ya existe un equipo registrado con el código 'EQ-001'.` y **no** se crea duplicado                                  | Ya existe un equipo registrado con el código 'EQ-001'. (No se creó duplicado) | **C**  |
| CP-03    | Negativo | —                        | Opción 1 → dejar el código vacío y pulsar Enter                               | Mensaje `Este dato no puede quedar vacío.` y el sistema vuelve a pedir el código                                             | Este dato no puede quedar vacio. (El sistema repite hasta recibir entrada)    | **C**  |
...
| ID       | Tipo     | Precondición            | Pasos    | Resultado esperado                                                                         | Resultado obtenido                                                         | Estado |
| -------- | -------- | ----------------------- | -------- | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- | ------ |
| CP-05 📷 | Positivo | 3 equipos disponibles   | Opción 2 | Se listan los 3 con código, tipo, marca, modelo y estado, y al final `Disponibles: 3 de 3` | ![Captura CP-05](Imagenes/2.png)                                           | **C**  |
| CP-06    | Positivo | EQ-001 prestado         | Opción 2 | EQ-001 aparece con estado *Prestado* y el conteo baja a `Disponibles: 2 de 3`              | ![Captura CP-06](Imagenes/3.png)                                           | **C**  |
| CP-07    | Borde    | `equipos.json` con `[]` | Opción 2 | Mensaje `Todavía no hay equipos registrados.` en lugar de una lista vacía                  | Efectivamente se muestra mensaje que indica que no hay equipos registrados | **C**  |

### HU03 — Registrar estudiante

| ID       | Tipo     | Precondición                   | Pasos                                                                                                | Resultado esperado                                                                                     | Resultado obtenido                                                                                             | Estado |
| -------- | -------- | ------------------------------ | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | ------ |
| CP-08 📷 | Positivo | 2 estudiantes registrados      | Opción 3 → `1011223344`, `Carlos Ruiz Vera`, `carlos.ruiz@campuslands.com`, `Desarrollo de Software` | Mensaje `Estudiante 'Carlos Ruiz Vera' registrado correctamente.` y aparece en `estudiantes.json`      | ![Captura CP-08](Imagenes/7.png) Queda en `estudiantes.json` con id 3. (El sistema no pide programa académico) | **C**  |
| CP-09    | Negativo | Documento 1098765432 ya existe | Opción 3 → documento `1098765432`                                                                    | Mensaje `Ya existe un estudiante registrado con el documento 1098765432.`                              | Ya existe un estudiante con el documento 1098765432. (No se creó duplicado)                                    | **C**  |
| CP-10    | Negativo | —                              | Opción 3 → documento `ABC123`                                                                        | Mensaje `El documento debe contener solo números.`                                                     | El documento debe contener solo numeros.                                                                       | **C**  |
| CP-11    | Negativo | —                              | Opción 3 → correo `correo-invalido`                                                                  | Mensaje `El correo no es válido (ejemplo: nombre@correo.com).`                                         | El correo no tiene un formato valido.                                                                          | **C**  |
...
| ID       | Tipo     | Precondición                         | Pasos                                                 | Resultado esperado                                                                             | Resultado obtenido                                                                                                                                                                                                                                  | Estado |
| -------- | -------- | ------------------------------------ | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| CP-13 📷 | Positivo | EQ-001 disponible y Laura registrada | Opción 4 → elegir EQ-001 → elegir a Laura Gómez       | Mensaje `Préstamo #1 registrado...`, se guarda la fecha de hoy y EQ-001 pasa a *Prestado*      | ![Captura CP-13](Imagenes/8.png)                                                                                                                                                                                                                    | **C**  |
...
| ID       | Tipo     | Precondición            | Pasos                            | Resultado esperado                                                                  | Resultado obtenido                            | Estado |
| -------- | -------- | ----------------------- | -------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------- | ------ |
| CP-19 📷 | Positivo | Préstamo #1 activo      | Opción 5 → elegir el préstamo #1 | Mensaje `Devolución registrada: el equipo 'EQ-001' vuelve a estar disponible.`      | ![Captura CP-19](Imagenes/10.png)             | **C**  |
...
| ID       | Tipo     | Precondición                   | Pasos    | Resultado esperado                                                                    | Resultado obtenido                                                                                      | Estado |
| -------- | -------- | ------------------------------ | -------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------ |
| CP-26 📷 | Positivo | 1 préstamo activo y 1 devuelto | Opción 7 | Aparecen **los dos**, cada uno con su estado y el devuelto con su fecha de devolución | ![Captura CP-26](Imagenes/CP-26.png) | **C**  |
| CP-27    | Positivo | Cerrar y reabrir el programa   | Opción 7 | El historial conserva los préstamos de la ejecución anterior                          | [Captura CP-27](Imagenes/CP-27.png) Se ejecutó el programa dos veces por separado; el historial mostrado fue idéntico en ambas ejecuciones. | **C**  |

### HU08 — Eliminar equipo

| ID       | Tipo     | Precondición      | Pasos                                       | Resultado esperado                                                                                          | Resultado obtenido                | Estado |
| -------- | -------- | ----------------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------- | ------ |
| CP-28 📷 | Negativo | EQ-001 prestado   | Opción 8 → elegir EQ-001 → confirmar `s`    | Mensaje `No se puede eliminar el equipo 'EQ-001' porque está prestado. Registre primero la devolución.`     | ![Captura CP-28](Imagenes/4.png)  | **C**  |
| CP-29 📷 | Positivo | EQ-001 disponible | Opción 8 → elegir EQ-001 → confirmar `s`    | Mensaje `Equipo 'EQ-001' eliminado del inventario.` y desaparece del listado                                | ![Captura CP-29](Imagenes/5.png)  | **C**  |
| CP-30    | Positivo | —                 | Opción 8 → elegir un equipo → responder `n` | Mensaje `Eliminación cancelada.` y el equipo sigue en el inventario                                         | ![Captura CP-30](Imagenes/12.png) | **C**  |
| CP-31    | Positivo | Tras CP-29        | Opción 7                                    | El historial **conserva** los préstamos del equipo eliminado                                                | ![Captura CP-31](Imagenes/13.png) | **C**  |

### Pruebas transversales

| ID    | Tipo  | Precondición                                | Pasos                                                         | Resultado esperado                                                                                                                                                   | Resultado obtenido                | Estado |
| ----- | ----- | ------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | ------ |
| CP-32 | Borde | —                                           | Menú principal → escribir `99`                                | Mensaje `Opción inválida, intente de nuevo.` y el menú vuelve a pedir la opción                                                                                      | ![Captura CP-32](Imagenes/14.png) | **C**  |
| CP-33 | Borde | —                                           | Menú principal → escribir `abc`                               | Mismo mensaje que CP-32; el programa no se cae                                                                                                                       | ![Captura CP-33](Imagenes/15.png) | **C**  |
| CP-34 | Borde | Editar `equipos.json` y dejarlo mal formado | Ejecutar `python main.py`                                     | Mensaje `El archivo ... está corrupto: ...` seguido de `Corrija el archivo antes de volver a ejecutar el programa.` — el programa termina sin traza de error         | ![Captura CP-34](Imagenes/16.png) | **C**  |
